...
<template>
  <div class="file-main">
    <arco-row justify="space-between" class="row-operate">
      <!-- 左侧区域 -->
      <arco-space wrap>
        <arco-form ref="queryRef" :model="queryParams" layout="inline">
          <arco-form-item hide-label>
            <arco-upload
            ...
            >
              <template #upload-button>
                <arco-button type="primary" shape="round">
          ...
          <arco-form-item field="name" hide-label>
            <arco-input
          ...
          <arco-form-item hide-label>
            <arco-space>
              <arco-button type="primary" @click="handleQuery">
                <template #icon><icon-search /></template>查询
              </arco-button>
              <arco-button @click="resetQuery">
      ...
      <arco-space wrap>
        ...
        <arco-button-group>
          <arco-tooltip content="视图" position="bottom">
            <arco-button @click="fileStore.changeViewMode">
              <template #icon>
          ...
          <arco-tooltip content="配置存储库" position="bottom">
            <arco-button
    ...
    <!-- 文件列表-宫格模式 -->
    <arco-spin class="file-wrap" :loading="loading">
      <FileGrid
      ...
      <arco-empty v-show="!fileList.length"></arco-empty>
    </arco-spin>
    ...
    >
      ...
      >
        <template #columns>
          <arco-table-column title="名称" :width="135">
...
<!--              <dict-tag :value="record.type" :dict="storage_type_enum" />-->
          ...
          <arco-table-column
            title="访问密钥"
            datarco-index="accessKey"
            ellipsis
            tooltip
          />
          <arco-table-column
            title="终端节点"
            datarco-index="endpoint"
            ellipsis
            tooltip
          />
          <arco-table-column
    ...
    >
      <arco-form
        ...
        layout="inline"
      >
        <arco-form-item label="名称" field="name">
          <arco-input
        ...
        <arco-form-item label="编码" field="code">
          <arco-input
        ...
        <arco-form-item label="类型" field="type">
          <arco-select
        ...
        <arco-form-item label="桶名称" field="bucketName">
          <arco-input
        ...
        <arco-form-item v-if="storageForm.type === 1" label="域名" field="domain">
          <arco-input
        ...
        <arco-form-item label="排序" field="sort">
          <arco-input-number
        ...
        <arco-form-item label="描述" field="description">
          <arco-textarea
        ...
        <arco-form-item label="默认存储" field="isDefault">
          <arco-switch v-model="storageForm.isDefault" type="round" />
        </arco-form-item>
      </arco-form>
    </arco-modal>
  </div>
